export class DockerfileTemplates {
    static getModernStructureDockerfile(phpVersion: string = '8.2'): string {
        return `# REDAXO Modern Structure Dockerfile
...
`;
    ...
    static getModernStructureReceipt(instanceName: string, phpVersion: string = '8.2', releaseType: string = 'modern'): string {
        return JSON.stringify({
            ...
            extensions: [
                'gd', 'pdo', 'pdo_mysql', 'mysqli', 'zip', 'intl', 'exif', 'imagick', 'xdebug'
            ],
            apacheModules: [
                'rewrite', 'headers', 'expires', 'ssl'
            ],
